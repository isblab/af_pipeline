...
data_path_1 = "tests/test_data/af_data_files/dummy_data_afdb.json"

data_path_2 = "tests/test_data/af_data_files/dummy_data_af3_1.json"

data_path_3 = "tests/test_data/af_data_files/dummy_data_af3_1.csv"

data_path_4 = "tests/test_data/af_data_files/dummy_data_af2.pkl"

data1 = DataParser(data_file_path=data_path_1).get_data_dict()
data2 = DataParser(data_file_path=data_path_2).get_data_dict()
data3 = {"not_valid_pae_key": [0, 1]}
data4 = DataParser(data_file_path=data_path_4).get_data_dict()

@pytest.fixture
def data_parser1():
    return DataParser(data_file_path=data_path_1)

@pytest.fixture
def data_parser2():
    return DataParser(data_file_path=data_path_2)

@pytest.fixture
def data_parser3():
    return DataParser(data_file_path=data_path_3)

@pytest.fixture
def data_parser4():
    return DataParser(data_file_path=data_path_4)

def test_data_type(
    data_parser1: DataParser,
    data_parser2: DataParser,
    data_parser3: DataParser,
    data_parser4: DataParser,
):
    """Test the DataParser.data_type property."""

    assert data_parser1.data_type == "json", \
        f"Data type should be 'json', got '{data_parser1.data_type}'."

    assert data_parser2.data_type == "json", \
        f"Data type should be 'json', got '{data_parser2.data_type}'."

    assert data_parser4.data_type == "pkl", \
        f"Data type should be 'pkl', got '{data_parser4.data_type}'."

    with pytest.raises(Exception):
        _ = data_parser3.data_type

def test_get_data_dicr(
    data_parser1: DataParser,
    data_parser2: DataParser,
    data_parser3: DataParser,
    data_parser4: DataParser,
    ...
    assert isinstance(data, dict), "Data should be a dictionary."
    assert "pae" in data, "Data should contain 'pae' key."
    assert "token_res_ids" in data, "Data should contain 'token_res_ids' key."
    assert "token_chain_ids" in data, "Data should contain 'token_chain_ids' key."
    assert "contact_probs" in data, "Data should contain 'contact_probs' key."

    data = data_parser4.get_data_dict()
    assert isinstance(data, dict), "Data should be a Dictionary."
    assert "plddt" in data, "Data should contain 'plddt' key."
    assert "predicted_aligned_error" in data, \
        "Data should contain 'predicted_aligned_error' key."
    # there are some other fields but we're not using them, so ignored.

    with pytest.raises(Exception):
        _ = data_parser3.get_data_dict()
...
def test_get_token_chain_ids():
    ...
    token_chain_ids = DataParser.get_token_chain_ids(data4)
    assert token_chain_ids is None, \
        "Token chain IDs should be None for AF2 .pkl files."
...
def test_get_token_res_ids():
    ...
    token_res_ids = DataParser.get_token_res_ids(data4)
    assert token_res_ids is None, \
        "Token residue IDs should be None for AF2 .pkl files."
...
def test_get_pae():
    ...
    pae = DataParser.get_pae(data4)
    assert isinstance(pae, np.ndarray), \
        "PAE should be a numpy array for AF2 .pkl files."

    with pytest.raises(Exception):
        pae = DataParser.get_pae(data3)
...
def test_get_contact_probs_mat():
    ...
    contact_probs = DataParser.get_contact_probs_mat(data4)
    assert contact_probs is None, \
        "Contact probabilities should be None for AF2 .pkl files."
...
def test_get_atom_chain_ids():
    ...
    atom_chain_ids = DataParser.get_atom_chain_ids(data4)
    assert atom_chain_ids is None, \
        "Atom chain IDs should be None for AF2 .pkl files."
...
def test_get_atom_plddts():
    ...
    atom_plddts = DataParser.get_atom_plddts(data4)
    assert atom_plddts is None, \
        "Atom plddts should be None for AF2 .pkl files."